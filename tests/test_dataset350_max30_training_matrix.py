...
def test_max30_changes_only_the_intended_training_budget_and_run_identity() -> None:
    ...
    assert max30["matrix"]["cases"] == baseline["matrix"]["cases"]
...
def test_max30_manifest_has_fifteen_isolated_eight_fold_jobs() -> None:
    ...
    text = render_matrix_training_jobs(
        MAX30_CONFIG.relative_to(ROOT), config, config["k8s"]["helper_image"]
    )
    ...
    assert all(f'OUT="{MAX30_ROOT}/{case}/month_${{TEST_START}}"' in text for case in cases)
    assert BASELINE_ROOT not in text
    assert MAX30_CONFIG.name in text
    assert text.count(f"image: {config['k8s']['helper_image']}") == 15
