...
- Next rolling target: 先用已完成的 `2021-2024` base / next-close / mixed-w030 cache 做
  ...
  smoke。36m smoke 不再跑 full-feature；使用 PVC 列投影、年度文件分块 transform、`downcast_float32`
  和 `soft_core_reg_light`，避免读取/特征构造/训练矩阵阶段接近 512Gi 内存上限。后续补齐
  `2015-2020` 后再把同一口径往更早 OOS 月份扩展。
