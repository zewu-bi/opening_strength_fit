# Archived Experiment Ledger (2026-05-20 to 2026-07-10)

> Archive status: frozen historical narrative.
> Authoritative reconciled timeline: [../experiment_log.md](../experiment_log.md).

本文件保留旧版逐日叙述、命令输出和中间判断，便于追溯当时上下文。它按“写入顺序”而非严格发生
时间排序，包含已被后续实验替代的下一步、过时状态和一处错误的归档政策描述。因此不要从本文件读取
当前状态、当前候选或结果目录政策；这些以新的 experiment log、run TOML 和 artifact trace 为准。
