...
| 2026-07-31 | `opening_{0931_0940,1001_1010,1401_1410}_next_close_clock6_rebuild` | 用三个 corrected decision-state cache 各自的 `T+6s` buy price 重建 next-close return，并补齐 v6 key 覆盖 | 三个 v6 base 均已 `7/7 completed`；三个 indexed Job 已同时提交，每个 `7/7` 并行，共 21 个年度 shard，均在 node15 启动 | 建立独立 corrected long-label lineage，不覆盖 legacy/v5 输出；完成后 mixed target 必须切换到对应新 root 才能消除旧入场价与缺 key 问题 |
