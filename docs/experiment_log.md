...
Note: 已归档的 baseline metrics/backtest 使用无成交延迟旧口径（`entry_tick_delay = 0`，
当前 tick ask1 买入）。当前实验配置已切到 `entry_tick_delay = 1`；后续新实验只继续
GBM 与 GBM strong，两类口径不要直接横向混比。
...
和 predictions，并按 runbook 生成 I500 sanity-check 回测。原始 metrics 与 backtest JSON 已用
...
能转化为明显正收益；普通 GBM 最强。此前 `aggregate=max`、`tar=I500` 的负结果主要反映
tick-level 信号被压成日频 score 后口径不匹配，不能直接否定这个短周期信号。
下一步先在 GBM / GBM strong 上加入成本、滑点、容量、重复持仓和成交约束。
