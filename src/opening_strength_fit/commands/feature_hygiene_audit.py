"""Feature-hygiene data loading and audit workflow."""
...
def _coerce_month_list(value: object) -> list[str]:
    return [str(pd.Period(item, freq="M")) for item in coerce_str_list(value)]
...
def _available_month_dates(files: list[Path], month: str) -> list[str]:
    start_date, end_date = _month_bounds(month)
    filters = [("date", ">=", start_date), ("date", "<=", end_date)]
    date_values = []
    for file in files:
        try:
            frame = _read_frame_file(file, columns=["date"], filters=filters)
...
def _read_selected_target_days(
    files: list[Path],
    columns: list[str],
    ...
    for date in selected_dates:
        filters = [("date", "==", date)]
        for file in files:
            part = _read_frame_file(file, columns=columns, filters=filters)
            if part.empty:
                continue
...
def _historical_target_keys(
    ...
    *,
    ...
    target_day_parts = [
        part
        for file in files
        if _file_overlaps_date_range(file, date, date)
        if not (part := _read_frame_file(file, columns=columns, filters=filters)).empty
    ]
    if not target_day_parts:
        return None
    target_day = pd.concat(target_day_parts, ignore_index=True)
    target_labeled = filter_labeled_frame(target_day, config)
    rows_before_sample = len(target_labeled)
    if sample_rows and len(target_labeled) > sample_rows:
        target_labeled = target_labeled.sample(
            n=sample_rows,
    ...
    key_columns = [
        ...
        if column in target_labeled.columns
    ]
    if not key_columns:
        return None
    target_keys = target_labeled[key_columns].drop_duplicates()
    symbols = (
        sorted(target_keys["symbol"].dropna().astype(str).unique().tolist())
        if "symbol" in target_keys.columns
        else []
    )
    return target_keys, rows_before_sample, symbols


def _historical_context_frame(
    files: list[Path],
    columns: list[str],
    *,
    context_start: str,
    ...
    parts = [
        part
        for file in files
        if _file_overlaps_date_range(file, context_start, date)
        if not (part := _read_frame_file(file, columns=columns, filters=filters)).empty
    ]
    if not parts:
        return None, 0
    ...
    return context, sum(len(part) for part in parts)


def _sample_historical_context(
    files: list[Path],
    columns: list[str],
    ...
    *,
    ...
    for date_index, date in enumerate(unique_target_dates):
        ...
        target = _historical_target_keys(
            files,
            columns,
            date,
            target_key_config,
            sample_rows=per_date_sample_rows,
            random_state=int(random_state) + date_index,
        )
        if target is None:
            continue
        target_keys, target_rows_before_sample, symbols = target
        context, raw_rows_for_date = _historical_context_frame(
            files,
            columns,
        ...
        merge_keys = [column for column in target_keys.columns if column in labeled.columns]
        if merge_keys:
        ...
        del target_keys, context, labeled, target_labeled
        gc.collect()
    print_mapping(
        "feature_hygiene_historical_context",
        {
            "lookback_days": historical_lookback,
            "target_dates": len(unique_target_dates),
...
def _sample_labeled_pvc_frame(args: argparse.Namespace, config: dict) -> pd.DataFrame:
    settings = CommandArguments(args, config, "feature_hygiene")
    ...
    days_per_month = settings.integer("days_per_month", 3)
    max_rows = settings.integer("sample_rows", 500_000)
    random_state = settings.integer("random_state", 7)
    if not sample_months:
        labeled = filter_labeled_frame(read_frame(path, columns=columns), config)
    ...
    for month in sample_months:
        dates = _available_month_dates(files, month)
        selected_dates = _choose_dates(dates, int(days_per_month))
        target_dates.extend(selected_dates)
        if not historical_lookback:
            parts.extend(_read_selected_target_days(files, columns, selected_dates, config))
        print_mapping(
    ...
    rows_before_sample = sum(len(part) for part in parts)
    ...
    print_mapping(
        "feature_hygiene_sample",
        {
            ...
            "rows_before_final_sample": rows_before_sample,
            "rows": len(labeled),
...
def _default_importance_path(args: argparse.Namespace, config: dict) -> Path | None:
    ...
    return next(
        (
            path
            for root in (Path("experiments/results/backtests"), Path("output/artifacts"))
            if (path := root / name / "feature_importance.csv").exists()
        ),
        None,
    )
...
def _add_feature_hygiene_args(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    ...
    value_args = (
        (
            ...
            None,
            None,
            "Comma/space separated YYYY-MM months. Defaults to every Nth test month.",
        ),
        (
            "sample-month-stride",
            int,
            None,
            "Use every Nth month from the configured test window. Default: 6.",
        ),
        (
            "days-per-month",
            int,
            None,
            "Evenly sample this many trading dates per sampled month. Default: 3.",
        ),
        ("sample-rows", int, None, "Final row cap after date sampling. Default: 500000."),
        ("random-state", int, None, None),
        ("corr-method", None, ["spearman", "pearson"], "Correlation method. Default: spearman."),
        (
            "corr-threshold",
            float,
            None,
            "Report/review pairs above this absolute correlation. Default: 0.98.",
        ),
        (
            "near-duplicate-threshold",
            float,
            None,
            "Hard-drop non-representatives above this absolute correlation. Default: 0.995.",
        ),
        (
            "min-corr-periods",
            int,
            None,
            "Minimum overlapping non-null observations for correlation. Default: 100.",
        ),
    )
    for name, converter, choices, help_text in value_args:
        add(f"--{name}", type=converter, choices=choices, default=None, help=help_text)
    add(
        "--cross-group",
        action="store_true",
...
def main(argv: list[str] | None = None) -> None:
    ...
    near_constant_top_ratio = settings.float("near_constant_top_ratio", 0.999)
    ...
    corr_method = settings.string("corr_method", "spearman")
    corr_threshold = settings.float("corr_threshold", 0.98)
    near_duplicate_threshold = settings.float("near_duplicate_threshold", 0.995)
    min_corr_periods = settings.integer("min_corr_periods", 100)
