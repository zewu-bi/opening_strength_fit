...
def _config_cases(config: dict, *, output_root: Path) -> list[DiagnosticCase]:
    section = config.get("execution_context", {})
    inputs = section.get("inputs", []) if isinstance(section, dict) else []
    if not isinstance(inputs, list):
        raise SystemExit("[execution_context].inputs must be an array of tables")
    cases = []
    for index, item in enumerate(inputs):
        if not isinstance(item, dict):
            raise SystemExit("each [[execution_context.inputs]] entry must be a table")
        ...
        if not selected_path or not prediction_root:
            raise SystemExit(
                "each [[execution_context.inputs]] entry requires selected_path and prediction_root"
            )
