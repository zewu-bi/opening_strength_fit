...
from opening_strength_fit.commands.delay_replay_runner import (
    plot_scenario_summary,
    replay_scenario,
    write_trace,
)
from opening_strength_fit.commands.delay_replay_validation import (
    delay_entry_ticks,
    missing_prediction_paths,
    validate_context_delay,
...
SCENARIOS: dict[str, ReplayScenario] = {
    ...
    "tradable_cost": ReplayScenario(
        name="tradable_cost",
        description=(
            "Cost plus continuous-trading status, decision lag <=5s, and entry tick-path gap <=10s."
        ),
        fee_bps=5.0,
        slippage_bps=5.0,
...
def main() -> None:
    ...
    for delay in delays:
        ...
        if missing:
            message = (
                f"{delay}: missing prediction files: {', '.join(str(path) for path in missing)}"
            )
