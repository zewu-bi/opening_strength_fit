...
def parse_csv_ints(value: str, *, default: Iterable[int]) -> list[int]:
    return (
        [int(part.strip()) for part in value.split(",") if part.strip()] if value else list(default)
    )


def parse_csv_strings(value: str, *, default: Iterable[str]) -> list[str]:
    return [part.strip() for part in value.split(",") if part.strip()] if value else list(default)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--prediction-root", default="/mnt/output/opening_strength_fit/nn")
    parser.add_argument(
        "--next-label-root",
        default=(
    ...
    parser.add_argument("--pool-path", default="lml.bzw@ssd/data/pool_L.parquet")
    parser.add_argument(
        "--output-dir",
    ...
    parser.add_argument("--variant", action="append", choices=sorted(DEFAULT_VARIANTS))
