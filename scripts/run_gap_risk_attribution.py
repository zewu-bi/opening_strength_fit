        ...
        specs.append(
            {
                ...
                "candidate_alpha_rank_min": float(
                    item.get("candidate_alpha_rank_min", 0.0) or 0.0
                ),
            }
        )
...
def selected_ids(group: pd.DataFrame, variant: dict[str, object], top_n: int) -> set[int]:
    ...
    risk_values = (
        pd.to_numeric(candidates[risk_col], errors="coerce").fillna(0.0)
        if risk_col
        else 0.0
    )
    candidates["final_score"] = (
        pd.to_numeric(candidates["candidate_alpha_rank"], errors="coerce")
        - penalty * risk_values
    )
    return set(
            ...
            rows.append(
                {
                    ...
                    "median_raw": float(
                        pd.to_numeric(part[feature], errors="coerce").median()
                    ),
...
def residual_penalized_vs_kept(
    ...
    *,
    ...
    wanted = membership.loc[
        membership["cohort"].isin(["baseline_kept", "penalized_out"])
    ].copy()
    columns = [
        "row_id",
        "next_excess_bps",
...
def main() -> None:
    ...
    prediction_dir = Path(
        args.prediction_dir
        or config_str(config, "attribution", "prediction_dir", "")
    )
    labeled_path = Path(
        args.labeled_path
        or config_str(config, "attribution", "labeled_path", "")
    )
    if not prediction_dir:
        raise SystemExit("gap attribution requires --prediction-dir or [attribution].prediction_dir")
    if not labeled_path:
    ...
    for month in months:
        ...
        frame = frame.dropna(subset=["label", "alpha_return_next_close", "candidate_alpha_rank"]).copy()
