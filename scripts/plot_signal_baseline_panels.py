...
STALE_OUTPUTS = (
    "next_close_top100_return_by_minute.png",
)
...
def summarize_by_minute(
    ...
    *,
    ...
    summary = (
        group_metrics.groupby("clock", as_index=False)
        .agg(
    ...
    summary["top100_excess_bps_sem"] = summary["top100_excess_bps_std"] / np.sqrt(
        summary["groups"]
    )
...
def main() -> None:
    ...
    print(
        short_summary[
            ["clock", "rank_ic_mean", "top100_excess_bps", "top100_mean_bps"]
        ].to_string(
            index=False,
            formatters={
